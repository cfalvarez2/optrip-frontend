...
export default function TripsDisplay() {
  ...
  const MaxTime = Number(state.MaxTime) * 60;
  ...
  useEffect(() => {
    if (count === 0) {
      if (TransportMean === "Avion") {
        getFlights();
      } else if (TransportMean === "Bus") {
        getBuses();
      } else {
        getBusesandFlights();
      }
  ...
  const getBuses = () => {
    ...
    axios.post("http://localhost:5000/bus_trips", JSON.parse(payload)).then((response) => {
  ...
  const getBusesandFlights = () => {
    const payload = JSON.stringify({ origin: from.codigo, destination: to.codigo, date: getFormattedDate(date) });
    axios
      .post("http://localhost:5000/flights", JSON.parse(payload), { headers: { "Content-Type": "application/json" } })
      .then((response) => {
        const flights = response.data.flights;
        let newArrayFlights = flights.map(function (ele: any) {
          return { ...ele, Flight: true };
      ...
      .then((response) => {
        const payloadBuses = JSON.stringify({
          origin: from.nombre,
          destination: to.nombre,
          date: getFormattedDate(date),
        });
        axios.post("http://localhost:5000/bus_trips", JSON.parse(payloadBuses)).then((response) => {
          const buses = response.data.bus_trips;
  ...
  const filterByPriceAndTime = (obj: any) => {
    return MaxTime !== 0 && MaxCost !== 0
      ? obj.cost <= MaxCost && obj.duration <= MaxTime
      : MaxCost !== 0
      ? obj.cost <= MaxCost
      : MaxTime !== 0
      ? obj.duration <= MaxTime
      : obj;
  };
  ...
  return !fetching ? (
    Trips && Trips.filter(filterByPriceAndTime).length > 0 ? (
      ...
      <Instruction>No hay viajes disponibles</Instruction>
    )
  ) : (
    <div>
      <LinearProgress />
